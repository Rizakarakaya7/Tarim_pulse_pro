...
def main():
    ...
    # --- DASHBOARD (STREAMLIT) BAŞLATMA ---
    # NOT: Docker'da 'subprocess.run' kullanılmalıdır ki konteyner açık kalsın.
    print("\n🚀 Dashboard sunucusu ayağa kaldırılıyor...")
    dashboard_path = os.path.join(BASE_DIR, "src", "dashboard", "app.py")
    
    try:
        # Streamlit'i modül olarak çağırıyoruz (-m streamlit)
        cmd = [
        ...
        # Bu işlem bloklayıcıdır; kullanıcı Streamlit'i kapatana kadar script burada bekler.
        # Docker konteynerinin kapanmamasını sağlar.
        subprocess.run(cmd, check=True)
        
    except Exception as e:
        print(f"[X] Dashboard başlatılırken hata oluştu: {e}")
        print(f"💡 Manuel başlatma komutu: python -m streamlit run {dashboard_path}")
